...
string default_wallet_url = "unix://" + (determine_home_directory() / "eonio-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
inline asset to_asset( const string& s ) {
   return to_asset( N(eonio.token), s );
}
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "eonio")("scope", "eonio")("table", "namebids")
   ...
   if (!json_format) {
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'eonio::refund' action)\n";
            } else {
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "eonio.msig", "propose", variant_to_bin( N(eonio.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "eonio.msig", action, variant_to_bin( N(eonio.msig), action, args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "eonio.msig", "invalidate", variant_to_bin( N(eonio.msig), "invalidate", args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "eonio.msig", "cancel", variant_to_bin( N(eonio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "eonio.msig", "exec", variant_to_bin( N(eonio.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send eonio.system contract action to the blockchain."), false);
