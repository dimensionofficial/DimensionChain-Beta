...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(eonio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(eonio.token), contracts::eosio_token_wasm() );
   c.set_abi( N(eonio.token), contracts::eosio_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( N(eonio.token), N(create), N(eonio.token), mutable_variant_object()
              ("issuer",       "eonio" )
   ...
   cr = c.push_action( N(eonio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
