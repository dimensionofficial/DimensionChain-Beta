...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(eonio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(eonio.token), contracts::eosio_token_wasm() );
         chain->set_abi(N(eonio.token), contracts::eosio_token_abi().data() );
         chain->push_action( N(eonio.token), N(create), N(eonio.token), mvo()
              ( "issuer", "eonio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(eonio.token), N(issue), N(eonio.token), mvo()
              ( "to", "eonio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(eonio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(eonio.token), N(alice)};
   test.init();

   test.transfer( N(eonio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(eonio.token), N(bob)};
   test.init();

   test.transfer( N(eonio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(eonio.token) );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(eonio.token), N(bob), N(charlie)};
