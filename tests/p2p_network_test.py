...
# eonio should have the same key as defproducera
eonio = copy.copy(defproduceraAccount)
eonio.name = "eonio"
...
wasmFile="eonio.system.wasm"
abiFile="eonio.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(eonio.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish eonio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, eonio)
