...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        self.eosioAccount=self.defProducerAccounts["eonio"]= Account("eonio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        self.eosioAccount=self.defProducerAccounts["eonio"]
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n EON and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse eonio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by eonio-launcher."""
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"eonio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init eonio transfer")
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        """Create 'prodCount' init accounts and deposits 10000000000 EON in each. If prodCount is -1 will initialize all possible producers.
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission eonio@active"
                    myTrans=biosNode.pushMessage("eonio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("eonio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but eonio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "eonio"
        ...
        eosioRamAccount.name="eonio.ram"
        ...
        eosioRamfeeAccount.name="eonio.ramfee"
        ...
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to eonio contract.")
            return None
        ...
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to eonio contract.")
            return None
        ...
        Utils.Print("Verify eonio issue, Expected: %s" % (expectedAmount))
