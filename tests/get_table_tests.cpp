...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   set_code( N(eonio.token), contracts::eosio_token_wasm() );
   set_abi( N(eonio.token), contracts::eosio_token_abi().data() );
   produce_blocks(1);

   // create currency
   auto act = mutable_variant_object()
         ("issuer",       "eonio")
         ("maximum_supply", eosio::chain::asset::from_string("1000000000.0000 EON"));
   push_action(N(eonio.token), N(create), N(eonio.token), act );

   // issue
   for (account_name a: accs) {
      push_action( N(eonio.token), N(issue), "eonio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", eosio::chain::asset::from_string("999.0000 EON") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(eonio.token), N(accounts), "inita", "", 10};
   ...
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(eonio.token)), result.rows[0].code);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      push_action( N(eonio.token), N(issue), "eonio", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", eosio::chain::asset::from_string("10000.0000 EON") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("10000.0000 EON", result.rows[3]["balance"].as_string());
   }
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("10000.0000 EON", result.rows[0]["balance"].as_string());
   }
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("10000.0000 EON", result.rows[0]["data"]["balance"].as_string());
      BOOST_REQUIRE_EQUAL("eonio", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("eonio", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("eonio", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("eonio", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   bidname(N(inita), N(com), eosio::chain::asset::from_string("10.0000 EON"));
   bidname(N(initb), N(org), eosio::chain::asset::from_string("11.0000 EON"));
   bidname(N(initc), N(io), eosio::chain::asset::from_string("12.0000 EON"));
   bidname(N(initd), N(html), eosio::chain::asset::from_string("14.0000 EON"));
   produce_blocks(1);
